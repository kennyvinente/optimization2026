...
function run_fonc_example()
    ...
    savefig(fig, joinpath(FIGURES_DIR, "fonc_example.png"))
    display(fig)
end
...
function run_quartic_problem()
    ...
    fig = plot(p1, p2; layout = (1, 2))
    savefig(fig, joinpath(FIGURES_DIR, "quadratic_surface_contour.png"))
    display(fig)
end
...
function run_cstr_lp()
    ...
    fig = plot(p_y, p_u; layout = (2, 1), size = (850, 750))
    savefig(fig, joinpath(FIGURES_DIR, "cstr_optimal_control.png"))
    display(fig)
