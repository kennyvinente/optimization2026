...
param b_eq{i in 1..ml+mq} := sum{j in 1..n} A[i,j]*xstar[j] 
 + if i > ml then 0.5* sum{j in 1..n, k in 1..n} P[i-ml,j,k]*xstar[j]*xstar[k];
param b_ineq{i in 1..pl+pq} :=
  sum{j in 1..n} A[ml+mq+i,j]*xstar[j] + if i > pl then
     0.5* sum{j in 1..n, k in 1..n} P[mq+i-pl,j,k]*xstar[j]*xstar[k];
...
s.t. eq{i in 1..ml+mq}:  sum{j in 1..n} A[i,j]*x[j] 
 + if i > ml then 0.5* sum{j in 1..n, k in 1..n} P[i-ml,j,k]*x[j]*x[k] == b_eq[i];
s.t. ineq{i in 1..pl+pq}:
  sum{j in 1..n} A[ml+mq+i,j]*x[j] + if i > pl then
     0.5* sum{j in 1..n, k in 1..n} P[mq+i-pl,j,k]*x[j]*x[k] <= b_ineq[i];
 data;
	# Sample randqp.dat; 
	param n := 500;
	param ml := 10;
