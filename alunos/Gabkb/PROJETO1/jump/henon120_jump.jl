...
# ============================================================
# henon120_jump.jl
# Tradução manual/estruturada do arquivo henon120.mod para JuMP
# ============================================================
...
mod_path = joinpath(@__DIR__, "..", "henon120.mod")

if !isfile(mod_path)
    mod_path = "henon120.mod"
end

if !isfile(mod_path)
    error("Arquivo henon120.mod não encontrado.")
end
...
# ------------------------------------------------------------
# Parâmetros específicos do Henon
# AMPL:
# let a := 1;
# let {n in N} b[n] := 0;
...
c = [sqrt(COORDS[n,1]^2 + COORDS[n,2]^2) for n in N]
...
println("Modelo henon120 carregado com sucesso.")
