...
b = JuMP.Containers.DenseAxisArray([((i <= ml) ? (sum((A[i,j]*xstar[j]) : (if i) for j in 1:(n))>ml && i <= ml+mq then sum((A[i,j]*xstar[j]) for j in 1:(n))+0.5* sum((P[i-ml,j,k]*xstar[j]*xstar[k] else if i) for j in 1:(n), k in 1:(n))>ml+mq && i <= ml+mq+pl then sum((A[i,j]*xstar[j]) for j in 1:(n))+ if z[i-ml-mq] == 0 then Uniform(-10,0) else if i > ml+mq+pl then sum((A[i,j]*xstar[j]) for j in 1:(n))+0.5* sum((P[i-ml-pl,j,k]*xstar[j]*xstar[k]) for j in 1:(n), k in 1:(n))+ if z[i-ml-mq] == 0 then Uniform(-10,0))) for i in 1:(ml+mq+pl+pq)], 1:(ml+mq+pl+pq))
...
# eq
for i in 1:(ml+mq)
    @NLconstraint(model, sum((A[i,j]*x[j]) for j in 1:(n))+ if i > ml then 0.5* sum((P[i-ml,j,k]*x[j]*x[k]) for j in 1:(n), k in 1:(n))=== b[i])
end

# ineq
for i in 1:(pl+pq)
    @NLconstraint(model, sum((A[ml+mq+i,j]*x[j]) for j in 1:(n))+ if i > pl then 0.5* sum((P[mq+i-pl,j,k]*x[j]*x[k]) for j in 1:(n), k in 1:(n))>= b[ml+mq+i])
end
