...
# ============================================================
# USO:
# julia --project=. rodar_um_ipopt.jl jump/bearing_400_jump.jl
# ============================================================

if length(ARGS) < 1
    error("Use assim: julia --project=. rodar_um_ipopt.jl caminho/do/modelo.jl")
end

arquivo = ARGS[1]
modelo_nome = splitext(basename(arquivo))[1]

pasta_logs = "logs_ipopt_simples"
mkpath(pasta_logs)

log_ipopt = joinpath(pasta_logs, modelo_nome * "_ipopt.log")

csv_resumo = "resultado_ipopt_simples.csv"
csv_iteracoes = "iteracoes_ipopt_simples.csv"
